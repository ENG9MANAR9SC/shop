<template>
  <div class="relative z-10">
    <button class="flex items-center space-x-2 gap-2 py-[2px] rounded-md text-secondery hover:bg-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
      @click="isOpen = !isOpen">
    ...
    <div class="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100" :class="[isOpen ? 'block' : 'hidden']">
		<template v-for="language in languages" :key="language">
			<button
			class="flex items-center justify-between w-full px-2 py-2 text-sm text-secondery hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" :class="{ 'font-medium': activeLanguage === language }"	@click="selectLanguage(language)">
			<img :src="languageIcons[language].icon" :alt="languageIcons[language].name" class="h-5 w-5" />
			<span class="gap-2">{{ languageIcons[language].name }}</span>
